 ...
 <body>
    ...
    <tr align="center" valign="middle" > 
        ...
        <td> 
            ...
            <a href="${pageContext.request.contextPath}/qaboardDetail/${row.board_num}">${row.board_subject}</a></div>                
        </td>         
        <td> 
            <div align="center">${row.board_id}</div> 
        </td> 
        <td>
	...
	<div class="pagination">
         <c:if test="${page>1}">
			<a href="${pageContext.request.contextPath}/qaboardlist.do?page=${page-1}">이전</a>
		</c:if>
		<c:forEach var="pno" begin="${startpage}" end="${endpage}">
		<c:choose>
        <c:when test="${pno==page}">
            <a href="${pageContext.request.contextPath}/qaboardlist.do?page=${pno}"><strong>${pno}</strong></a>
       </c:when>
        <c:otherwise>
       		<a href="${pageContext.request.contextPath}/qaboardlist.do?page=${pno}">${pno}</a>
        </c:otherwise>
    </c:choose>
		</c:forEach>
		<c:if test="${page<maxpage}">
			<a href="${pageContext.request.contextPath}/qaboardlist.do?page=${page+1}" class="direction">다음</a>
		</c:if>
    </div>
	</td>
	</tr>
	 ...
	 <tr align="right"> 
        <td class="write" colspan="5"> 
     		<a href="${pageContext.request.contextPath}/qaboardwrite.do"><img src="${pageContext.request.contextPath}/resources/images/btn_m.jpg"></td></a>
       </tr> 
</table>
		</div>
		<div class="footer">
